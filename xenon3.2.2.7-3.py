...
# Define the gesture thresholds
pinky_threshold = 0.9
thumb_threshold = 0.9
mid_threshold = 0.8
first2_threshold = 0.8
...
class HandRecog:
    ...
    def get_gesture(self):
        #if self.hand_result < thumb_threshold and self.hand_result < mid_threshold and self.hand_result< first2_threshold and self.hand_result >= pinky_threshold:
        #if self.hand_result< pinky_threshold:
        """if Gest.PINKY == self.finger:
            point = [[8, 20], [5, 17]]
        ...
        if self.finger==Gest.PINKY and self.get_dist([20, 19]) < 0.1:
            current_gesture=Gest.PINKY

        elif self.hand_result == None:
            return Gest.PALM
...
class Controller:
    ...
    # enable windows onscreen keyboard
